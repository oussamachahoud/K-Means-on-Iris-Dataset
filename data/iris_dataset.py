...
def load_iris_dataset() -> np.ndarray:
    ...
    Returns:
        np.ndarray: مصفوفة البيانات
    """
    return np.array(IRIS_DATA)


def get_dataset_info(data: np.ndarray) -> dict:
    """
    الحصول على معلومات عن مجموعة البيانات
    ...
    return {
        ...
        'min_values': data.min(axis=0).tolist(),
        'max_values': data.max(axis=0).tolist(),
        'mean_values': data.mean(axis=0).tolist()
    }